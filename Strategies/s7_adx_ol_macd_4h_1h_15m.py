...
class s7_ADX_OL_MACD_4h_1h_15m(Strategy):
    ...
    def __init__(self, config2tf):
        ...
        self.return_15m_dict =defaultdict(list)

        #Setting 1Hr Parameters
        self.Config2tf_1h_Obj = Config2tf(self.config2tfObjPtr.get_args())
        self.Config2tf_1h_Obj.timeframe = self.config2tfObjPtr.second_timeframe
        self.Config2tf_1h_Obj.run()
        self.flag_1h = 0
        self.return_1h_dict=defaultdict(list)

        #Setting 4Hr Parameters
        self.Config2tf_4hr_Obj = Config2tf(self.config2tfObjPtr.get_args())
        self.Config2tf_4hr_Obj.timeframe = self.config2tfObjPtr.third_timeframe
    ...
    def run(self):
        ...
        self.return_15m_dict ={}

        if((self.flag_1h % 4)==0) :
            self.return_1h_dict={}

        if((self.flag_4hr % 16)==0) :
        ...
        for symbol in self.config2tfObjPtr.all_symbols:
            ...
            self.df_current = self.symbolObj.read_record_wchks(self.config2tfObjPtr)
            self.return_15m_dict[symbol] = self.ADX_OL_MACD(self.df_current)

            ###1h_Params
            self.Config2tf_1h_Obj.symbol = symbol
            self.Config2tf_1h_Obj.make_symbol_data_paths()
            if((self.flag_1h % 4)==0) :
                df_1h = self.symbolObj.read_record_wchks(self.Config2tf_1h_Obj)
                self.return_1h_dict[symbol] = self.ADX_MACDHist(df_1h)

            ###4hr_Params
            self.Config2tf_4hr_Obj.symbol = symbol
            self.Config2tf_4hr_Obj.make_symbol_data_paths()
            if((self.flag_4hr % 16)==0) :
                df_4hr = self.symbolObj.read_record_wchks(self.Config2tf_4hr_Obj)
                self.return_4hr_dict[symbol] = self.ADX_MACDHist(df_4hr)

            self.config2tfObjPtr.return_bs_dict[symbol] = self.bs_intersection(self.return_4hr_dict[symbol], self.return_1h_dict[symbol], self.return_15m_dict[symbol])

        self.config2tfObjPtr.strategy_name = self.__class__.__name__
        self.config2tfObjPtr.timestamp = self.df_current['Timestamp'].iloc[-2]
        self.tg_logger(self.config2tfObjPtr)
        self.flag_1h=self.flag_1h+1
        self.flag_4hr=self.flag_4hr+1

        self.config2tfObjPtr.strategy_name = 's4_adx_ol_macd_4h_1h_15m:15m_dict'
        self.config2tfObjPtr.return_bs_dict = self.return_15m_dict
        self.tg_logger(self.config2tfObjPtr)

        self.Config2tf_1h_Obj.strategy_name = 's4_adx_ol_macdhist_4h_1h_15m:1h_dict'
        self.Config2tf_1h_Obj.return_bs_dict = self.return_1h_dict
        self.tg_logger(self.Config2tf_1h_Obj)

        self.Config2tf_4hr_Obj.strategy_name = 's4_adx_ol_macdhist_4h_1h_15m:4hr_dict'
        self.Config2tf_4hr_Obj.return_bs_dict = self.return_4hr_dict
        self.tg_logger(self.Config2tf_4hr_Obj)

    def bs_intersection(self, list1, list2, list3):
        out0=list1[0] & list2[0] & list3[0]
        out1=list1[1] & list2[1] & list3[1]
        return [out0, out1]
    ...
    def ADX_OL_MACD(self, df):
        ...
        def adx5_14_ol_macd_cond(row, columns):
            ...
            histA_IsUp = (row['histLine'] > 0)
            histB_IsDown = (row['histLine'] <= 0)
        ...
        return [df['SELL'].iloc[-1], df['BUY'].iloc[-1], df['Timestamp'].iloc[-1]]
