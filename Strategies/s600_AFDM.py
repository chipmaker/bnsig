...
class s600_AFDM(object):
    ...
    def AFDM(self, df):
        ...
        df['mBCSH'] = df['mBC'].shift(1, fill_value=False)
        df['mSCSH'] = df['mSC'].shift(1, fill_value=False)
        df['mBCSH2'] = df['mBC'].shift(2, fill_value=False)
        df['mSCSH2'] = df['mSC'].shift(2, fill_value=False)
        ...
        columns = ['aBC', 'aSC', 'dBC', 'dSC', 'mBC', 'mSC', 'mBCSH', 'mSCSH', 'mBCSH2', 'mSCSH2']

        def admCond(row, columns):
            buy = row['aBC'] & row['dBC'] & (row['mBC'] | row['mBCSH'] | row['mBCSH2'])
            sell = row['aSC'] & row['dSC'] & (row['mSC'] | row['mSCSH'] | row['mSCSH2'])
            return buy, sell
