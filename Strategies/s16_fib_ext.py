...
class s16_FIB_EXT(object):

    def __init__(self):
        self.strategy_name = 'S16_FIB_EXT'
        logger.info("Initializing s16_FIB_EXT")
        """Constructor for $class$"""
        self.withinBars = 1
        self.udShow = 0
        self.hPrice = -1 #-1 trending down, 1 trending up
        self.lPrice = -1 #-1 trending down, 1 trending up

    def run(self, df, row):
        self.hPrice = row['FibHigh'] #contraints[0] #FibHigh
        self.lPrice = row['FibLow']  #contraints[1] #FibHigh
        if((math.isnan(self.hPrice)) | (math.isnan(self.lPrice)) | ((self.hPrice < 0) | (self.lPrice < 0)|(self.hPrice < self.lPrice))) :
            print("######### Quitting. check hprice={} and lprice={}".format(self.hPrice, self.lPrice))
            quit()
    ...
    def FIB_EXT(self, df):
        ...
        pricehigh = self.hPrice
        pricelow = self.lPrice
        ...
        def fib_cond(row, columns):
            ...
            sell = (fibdirection == -1) & \
                   ((co11p089 | co6p853andhalf | co6p853 | cu11p089 | cu6p853andhalf | cu6p853) | \
                    ...
                    (cu4p236 | cu3p618 | cu2p618 | cu1 | cu0))
            buy = (fibdirection == 1) & \
                  ((co11p089 | co6p853andhalf | co6p853 | cu11p089 | cu6p853andhalf | cu6p853) | \
                   (cu4p236andhalf | co4p236andhalf) | \
                   (co4p236 | co3p618 | co2p618 | co1 | co0))
